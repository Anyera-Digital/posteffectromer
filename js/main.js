...
if(productsinput){
  ...
  document.querySelectorAll('.products__count .products__plus').forEach(function (element) {
    element.addEventListener('click', function(event) {
      ...
      var maxVal = inputMax ? parseInt(inputMax, 10) : 999;
  ...
  document.querySelectorAll('.products__count .products__input input').forEach(function (element) {
    element.addEventListener("change", function(event) {
      ...
      var maxVal = inputMax ? parseInt(inputMax, 10) : 999;
      if (!isNaN(maxVal) && this.value > maxVal) {
        this.value = maxVal;
      }
    });
  });
...
function headerScrollThreshold() {
  if (!headertop) return 0;
  const accent = document.querySelector('.header__accent');
  const info = document.querySelector('.header__info');
  const edge = document.querySelector('.header__info_edge');
  if (!accent || !info || !edge) {
    return headertop.clientHeight;
  }
  const hidden = window.getComputedStyle(accent).display === 'none';
  if (hidden) {
    return headertop.clientHeight;
  }
  return (accent.offsetHeight || 0) + (info.offsetHeight || 0) + (edge.offsetHeight || 0);
}
if (header && headertop && window.innerWidth > 1280 && !header.classList.contains('header--cart')) {
  window.addEventListener("scroll", () => {
    if (Math.round(window.pageYOffset) > headerScrollThreshold()) {
...
if (burger && menu && overlay) {
  ...
  document.querySelectorAll('.header__nav_simple_link, .header__nav_simple_sublink').forEach(function (navLink) {
  ...
  document.querySelectorAll('.header__nav_simple_toggle').forEach(function (toggle) {
    toggle.addEventListener('click', function () {
      ...
      sub.hidden = !next;
      const wrap = toggle.closest('.header__nav_simple_item--accordion');
...
// start hover products - show only 2nd image on hover
var hoverItems = document.querySelectorAll('.products__item');
hoverItems.forEach(function(item) {
  var images = item.querySelector('.products__images');
  if (!images) return;
  var imagesList = images.querySelectorAll('.products__image');
  if (imagesList.length < 2) return;

  item.addEventListener('mouseenter', function() {
    imagesList[0].classList.remove('active');
    imagesList[1].classList.add('active');
  });
  item.addEventListener('mouseleave', function() {
    imagesList[0].classList.add('active');
    imagesList[1].classList.remove('active');
  });
});
// end hover products
...
// start catalog_subsection filter (sidebar from artelectro)
...
if (csff) {
  ...
  for (let i = 0; i < csfb.length; i++) {
    csfb[i].onclick = function () {
  ...
  const rangesliderCs = document.getElementById('filterPrice');
  const rangesfilterInputs = document.querySelectorAll('.catalog_subsection__filter_range_input');
  const frclearCs = document.querySelector('.catalog_subsection__filter_clear');
  const csffInputs = [...document.querySelectorAll('.catalog_subsection__filter_form .catalog_subsection__filter_input')];
  if (rangesliderCs) {
    const rangeMin = parseInt(rangesliderCs.dataset.min);
    const rangeMax = parseInt(rangesliderCs.dataset.max);
    const rangestep = parseInt(rangesliderCs.dataset.step);
    noUiSlider.create(rangesliderCs, {
      start: [rangeMin, rangeMax],
      connect: true,
      step: rangestep,
      range: { min: Math.round(rangeMin), max: Math.round(rangeMax) },
      format: { to: value => Math.round(value), from: value => Math.round(value) }
    });
    rangesliderCs.noUiSlider.on('update', (values, handle) => {
      if (rangesfilterInputs[handle]) rangesfilterInputs[handle].value = values[handle];
    });
    rangesfilterInputs.forEach((input, indexInput) => {
      input.addEventListener('change', () => {
        rangesliderCs.noUiSlider.setHandle(indexInput, input.value);
      });
    });
    if (frclearCs) {
      frclearCs.addEventListener('click', function () {
        rangesliderCs.noUiSlider.reset();
        csffInputs.forEach(inp => { inp.checked = false; });
      });
    }
  }
}
// end catalog_subsection filter
...
if (!rangeslider || document.querySelector('.catalog_subsection__filter_form')) {} else {
...
class Select {
  ...
  isOpened() {
    return this.selectList.classList.contains('filter__select_list_opened') || this.selectList.classList.contains('form__select_list_opened');
  }
  openSelectList() {
    this.selectList.style.maxHeight = this.selectList.scrollHeight + "px";
    const isFormSelect = this.select.closest('.form__select');
    this.selectList.classList.add(isFormSelect ? 'form__select_list_opened' : 'filter__select_list_opened');
    this.selectArrow.classList.add(isFormSelect ? 'form__select_arrow_rotate' : 'filter__select_arrow_rotate');
  }
  closeSelectList() {
    this.selectList.style.maxHeight = null;
    this.selectList.classList.remove('filter__select_list_opened', 'form__select_list_opened');
    this.selectArrow.classList.remove('filter__select_arrow_rotate', 'form__select_arrow_rotate');
  }
...
if(pinformation){
  pdescrbutton.addEventListener('click', function() {
    if (!this.classList.contains("active")) {
      ...
      if (preview && !preview.classList.contains("active")) {
        ...
        if (previewform) { previewform.classList.remove("active"); previewform.style.maxHeight = null; }
      }
    }
  })
  ...
  if (previewsbutton) previewsbutton.addEventListener('click', function() {
    if (!this.classList.contains("active")) {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    if (preview && preview.classList.contains("active")) {preview.style.maxHeight = (preview.scrollHeight * 1) + "px";}
    ...
    if (previews && previews.classList.contains("active")) {previews.style.maxHeight = (previews.scrollHeight * 1) + "px";}
  });
  ...
  if (pibutton && previewform && previews) pibutton.addEventListener('click', function() {
    if (preview && preview.classList.contains("active")) {
      ...
      previews.style.maxHeight = null;
      if (preview) { preview.classList.add("active"); preview.style.maxHeight = (preview.scrollHeight * 1) + "px"; }
    }
  })
}
